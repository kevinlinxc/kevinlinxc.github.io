...
varying float vLight,vHeight,vPocket,vOccupied;
...
export default function TideField(){
 ...
 useEffect(()=>{
  ...
  const uniforms={uTime:{value:0},uAspect:{value:1},uScroll:{value:0},uReady:{value:0},uCalm:{value:0},uStory:{value:0},uCard:{value:new THREE.Vector4(8,8,.6,.4)},uResolve:{value:0},uPresence:{value:0},uPointer:{value:new THREE.Vector2()},uTypeCenter:{value:new THREE.Vector2()},uFilm:{value:filmTexture},uRatio:{value:1},uLayer:{value:0}};
  const frontUniforms={...uniforms,uLayer:{value:1}};
  const camera=new THREE.Camera(),scene=new THREE.Scene();
  const geometry=new THREE.BufferGeometry(),veilGeometry=new THREE.BufferGeometry();
  ...
  const resize=()=>{
   ...
   const grid=new Float32Array(nx*ny*2),positions=new Float32Array(nx*ny*3),veilGrid:number[]=[];
  ...
  function render(now:number){
   ...
   uniforms.uPresence.value+=((activeCard? .6:0)-uniforms.uPresence.value)*(frozen?1:smoothing);
   if(activeCard){
  ...
  return()=>{disposed=true;invalidate.current=()=>{};cancelAnimationFrame(frame);observer.disconnect();window.removeEventListener('pointermove',move);window.removeEventListener('focusin',focus);window.removeEventListener('scroll',requestRender);document.removeEventListener('visibilitychange',visibility);reduced.removeEventListener('change',requestRender);video.removeEventListener('loadedmetadata',ready);video.removeEventListener('loadeddata',requestRender);video.removeEventListener('seeked',requestRender);video.removeEventListener('error',videoError);video.pause();video.removeAttribute('src');video.load();player.current=null;geometry.dispose();veilGeometry.dispose();material.dispose();veilMaterial.dispose();backgroundGeometry.dispose();backgroundMaterial.dispose();filmTexture.dispose();base.dispose();base.domElement.remove();};
 },[]);
