...
function mapOutsideCode(body, transform) {
  return body
    .split(/(```[\s\S]*?```|~~~[\s\S]*?~~~)/)
...
function convertShortcodes(body) {
  return body
    .replace(/\{\{<\s*(?:youtube|youtubegif)\s+([^>]+?)\s*>\}\}/g, (_, id) => {
...
function copyImages(body, sourceDir, assetKey) {
...
function buildDoc({ file, sourceDir, slug, title, section }) {
  ...
  const html = marked.parse(withImages, { mangle: false, headerIds: false });
  return {
    slug,
